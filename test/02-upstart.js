...
var upstart = require('../index').upstart
...
describe('upstart', function () {
  ...
  it('writes stuff', function (done) {
    upstart(path.join(__dirname, '/fixtures/test1.toml'), { output: '.' }, function () {}, function (err) {
  ...
  it('handles objects', function (done) {
    upstart(path.join(__dirname, '/fixtures/user-acl-two.toml'), { output: '.' }, function () {}, function (err) {
      demand(err).not.exist()
